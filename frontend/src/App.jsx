...
class App extends Component {
  ...
  refreshData = async (query) => {
    var uri = '/catalog';
    if (query) {
      uri = `/catalog/search?q=${query}`;
    }

    const catalogPromise = fetch(uri)
      .then(res => res.json())
      ...
      <div className="TitleList">
        <div className="Title">
          <div className="TitleList__slider">
            <input type="text" id="search" name="search" className="Search__input" placeholder="Search movies" onChange={evt => onSearch(evt.target.value)} />
          </div>
        </div>
      </div>);
  }
